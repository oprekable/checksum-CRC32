...
// From https://raw.githubusercontent.com/rclone/rclone/master/bin/cross-compile.go
...
func compileArch(version, goos, goarch, dir string) bool {
	...
	output := filepath.Join(dir, "checksum-CRC32")
	if goos == "windows" {
		output += ".exe"
	}
	...
	args := []string{
		"go", "build",
		"--ldflags", "-s -X github.com/oprekable/checksum-CRC32/fs.Version=" + version,
		"-i",
		"-o", output,
		"-tags", *tags,
		"..",
...
func compile(version string) {
	...
	for _, osarch := range osarches {
		...
		dir := filepath.Join("checksum-CRC32-" + version + "-" + userGoos + "-" + goarch)
		run <- func() {
...
func main() {
	...
	chdir("build")
	err := ioutil.WriteFile("version.txt", []byte(fmt.Sprintf("checksum-CRC32 %s\n", version)), 0666)
	if err != nil {
